...
class AddChatRoomScreen extends StatefulWidget{
  ...
  @override
  State<AddChatRoomScreen> createState() {
    return _AddChatRoomScreenState();
  }
...
class _AddChatRoomScreenState extends State<AddChatRoomScreen>{
  ...
  void _submitButton() async{
    ...
    } else { //Joining a ChatRoom
        final doc = await FirebaseFirestore.instance.collection('chatRoom').doc(chatRoomId).get();
        final bool doesDocExist = doc.exists;
        if (!doesDocExist){
          _closeBottomSheet();
          ScaffoldMessenger.of(context).clearSnackBars();
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('No Such room Key Exists'),
            backgroundColor: Colors.red,
            duration: Duration(seconds: 3),
          ));
          return;
